...
class ClanCreationView(ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if leader:
            try: await leader.add_roles(clan_role)
            except: await channel.send("⚠️ I could not give the leader the clan role (My role might be below theirs).")
    ...
    async def reject(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if leader:
            try: await leader.send(f"❌ Your request to create the clan **{clan_name}** was rejected.")
            except: pass
...
class MemberApplicationView(ui.View):
    ...
    async def deny(self, interaction: discord.Interaction, button: ui.Button):
        ...
        try:
            ...
            applicant = interaction.guild.get_member(applicant_id)
            if applicant:
                try: await applicant.send(f"❌ Your application to join **{clan_data['name']}** was denied.")
                except: pass
        except: pass
...
class ClanTransferView(ui.View):
    ...
    async def get_transfer_details(self, interaction):
        ...
        channel_id = str(footer_parts[0].split(": ")[1])
        new_leader_id = int(footer_parts[1].split(": ")[1])
...
class ClanSystem(commands.Cog):
    ...
    async def apply_clan(self, interaction: discord.Interaction, clan_name: str, message: str):
        ...
        if interaction.user.id in target_clan["members"]: return await interaction.response.send_message("❌ You are already in this clan.", ephemeral=True)

        clan_channel = interaction.guild.get_channel(target_clan["channel_id"])
        if not clan_channel: return await interaction.response.send_message(f"❌ Channel for **{clan_name}** deleted.", ephemeral=True)
    ...
    async def transfer_ownership(self, interaction: discord.Interaction, new_leader: discord.Member):
        ...
        # FOOTER IS CRITICAL: Stores Channel ID and New Leader ID for the button to read
    ...
    async def clan_kick(self, interaction: discord.Interaction, member: discord.Member, reason: str):
        ...
        await interaction.response.send_message(f"👢 **{member.display_name}** kicked.\n**Reason:** {reason}")
